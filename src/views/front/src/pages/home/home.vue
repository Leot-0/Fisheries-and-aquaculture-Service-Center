...
<!-- 特价商品 -->
<div id="animate_listtiwenxinxi" class="lists animate__animated" :style='{"margin":"0 0 20px","background":"#fff","order":"4"}'>
	...
	<div class="title" :style='{"width":"100%","margin":"30px auto","lineHeight":"50px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20230915/b96d1e15768f4ac495e60a79e04cb731.png) no-repeat center top"}'>
	  <span :style='{"color":"#F5BB00","fontSize":"32px","lineHeight":"40px"}'>提问信息展示</span>
	</div>
	...
	<div v-if="tiwenxinxiList.length" class="list list11 index-pv1" :style='{"width":"100%","padding":"0 7%","background":"#fff","justifyContent":"space-between","display":"flex","height":"auto"}'>
	  <div class="list" :style='{"flexWrap":"wrap","flex":"1","background":"#fff","display":"flex","height":"auto"}'>
	    <div :style='{"cursor":"pointer","width":"31.3%","margin":"0 2% 20px 0","overflow":"hidden","position":"relative","height":"auto"}' class="item list-item animation-box" v-for="item,index in tiwenxinxiList" :key="index" @click="toDetail('tiwenxinxiDetail', item)">
			<img :name="item.id" :style='{"width":"100%","objectFit":"contain","borderRadius":"6px","display":"block","height":"200px"}' v-if="preHttp(item.fengmian)" :src="item.fengmian.split(',')[0]" alt="" />
			<img :name="item.id" :style='{"width":"100%","objectFit":"contain","borderRadius":"6px","display":"block","height":"200px"}' v-else :src="baseUrl + (item.fengmian?item.fengmian.split(',')[0]:'')" alt="" />
			<div class="line1" :style='{"padding":"0 10px","borderColor":"#ddd","whiteSpace":"nowrap","color":"#333","textAlign":"center","overflow":"hidden","borderWidth":"0px","width":"100%","lineHeight":"30px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid","height":"30px"}'>{{item.biaoti}}</div>
	    </div>
	  </div>
	</div>
	...
	<div @click="moreBtn('tiwenxinxi')" :style='{"border":"0","cursor":"pointer","margin":"30px auto 10px","borderRadius":"6px","textAlign":"center","background":"#F5BB00","display":"block","width":"120px","lineHeight":"36px"}'>
		<span :style='{"color":"#f5f5f5","fontSize":"14px"}'>查看更多</span>
    ...
    methods: {
		...
		handleScroll() {
			let arr = [
				...
				{id:'animate_listtiwenxinxi',css:'animate__fadeInUp'},
      ...
      preHttp(str) {
		...
		getList() {
			...
			this.$http.get('tiwenxinxi/list', {params: data}).then(res => {
				if (res.data.code == 0) {
					this.tiwenxinxiList = res.data.data.list;
