...
export default {
	data() {
		...
		return {
 yyy:JSON.parse(localStorage.getItem("userForm")),
			id: '',
			type: '',
       ...
       if(this.type=='cross'){
		...
		this.$refs["ruleForm"].validate(valid => {
			if (valid) {
              ...
              let xxx={
                userid:this.yyy.id,
                avatarurl:this.yyy.touxiang,
                nickname:this.yyy.renyuanxingming,
                content:this.ruleForm.reply,
                iftiwen:"1",
                ifreply:this.id
              }
							this.$http({
								url: `discusstiwenxinxi/save`,
								method: "post",
								data: xxx
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discusstiwenxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
