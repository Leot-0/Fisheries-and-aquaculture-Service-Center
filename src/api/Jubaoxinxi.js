...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let jubaorenzhanghao = req.query.jubaorenzhanghao
			if (jubaorenzhanghao) {

				if (jubaorenzhanghao.indexOf('%') != -1) {
					where.jubaorenzhanghao = {
						[Op.like]: jubaorenzhanghao
					}
				} else {
					where.jubaorenzhanghao = {
						[Op.eq]: jubaorenzhanghao
					}
				}
			}
			let jubaorenxingming = req.query.jubaorenxingming
			if (jubaorenxingming) {

				if (jubaorenxingming.indexOf('%') != -1) {
					where.jubaorenxingming = {
						[Op.like]: jubaorenxingming
					}
				} else {
					where.jubaorenxingming = {
						[Op.eq]: jubaorenxingming
					}
				}
			}
            ...
            if(sessionInfo['tableName'] == 'yonghu') {
                orwhere.push({ jubaorenzhanghao: sessionInfo['username'] })
            }
            if(orwhere.length > 0) {
                where[Op.or] = orwhere
            }
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await JubaoxinxiModel.findAll()
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM jubaoxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM jubaoxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM jubaoxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM jubaoxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jubaoxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jubaoxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM jubaoxinxi " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM jubaoxinxi " + where + " GROUP BY " + xColumnName + " DESC"
	...
	api.post('/shBatch', async (req, res) => {
		...
		try {
			...
			req.body.forEach(async id => {
				let data= await JubaoxinxiModel.findOne({ where: { id } })
				data = data.dataValues
				data.sfsh = req.query.sfsh
				data.shhf = req.query.shhf
				await JubaoxinxiModel.update(data, {
					where: {
						id: data.id || 0
					}
				})
